  ...
  const HomePage: React.FC<HomePageProps> = ({ currentLang }) => {
    ...
    // Callback appelé quand le contenu 3D (Suspense) est résolu
    const onCanvasReady = useCallback(() => {
      // Délai pour garantir que la transition de Framer est bien lancée
      setTimeout(() => {
        setIs3dLoaded(true);
      }, 100); 
    }, []);

    // Utilisation de useEffect pour déclencher 'onCanvasReady' après le montage réussi
    // (qui se produit après que Suspense a résolu le chargement du GLTF).
    useEffect(() => {
      if (!is3dLoaded) {
        onCanvasReady();
      }
    ...
    return (
      <div className="w-full h-screen relative">
        {/* 1. Canvas 3D (Contrôle d'opacité) */}
        <Canvas 
          className="w-full h-full"
          // Affichage progressif du Canvas après chargement 3D
          style={{ opacity: is3dLoaded ? 1 : 0, transition: 'opacity 0.7s ease-in-out' }}
          camera={{ position: [0, 0, 5], fov: 45 }}
        >
          {/* Suspense pour le chargement du modèle GLTF */}
          <Suspense fallback={<group />}>
            <group>
              {/* UTILISATION DU COMPOSANT FOND RÉUTILISABLE (importé) */}
              <SpaceBackground /> 
        ...
        {/* 2. Masque de Chargement (Cache le 3D tant qu'il n'est pas prêt) */}
        {!is3dLoaded && (
